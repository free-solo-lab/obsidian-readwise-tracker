...
var ReadwiseService = class {
  ...
  async request(endpoint, params = {}) {
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      if (response.status === 429) {
        const retryAfterRaw = (_c = (_a = response.headers) == null ? void 0 : _a["Retry-After"]) != null ? _c : (_b = response.headers) == null ? void 0 : _b["retry-after"];
        const retryAfterSeconds = retryAfterRaw ? Number.parseInt(retryAfterRaw, 10) : 5;
        const waitMs = Number.isFinite(retryAfterSeconds) ? Math.max(1, retryAfterSeconds) * 1e3 : 5e3;
        if (this.debug) {
  ...
  async getAllDocuments(location, category, updatedAfter) {
    let allDocuments = [];
...
var StatsEngine = class {
  ...
  getReadingStats() {
    ...
    const wpm = 200;
    const completedMinutes = books.filter((b) => b.status === "completed").reduce((sum, b) => sum + (b.words_count || 0) / wpm, 0);
    const remainingMinutes = books.filter((b) => b.status === "reading").reduce((sum, b) => {
  ...
  getBooksByStatus(status) {
    return Object.values(this.dataManager.getData().books).filter((b) => b.status === status);
  }
  getPlanProgress(planId) {
    ...
    if (totalBooks === 0)
      return 0;
    const completedInPlan = plan.book_ids.map((id) => this.dataManager.getBook(id)).filter((book) => book && book.status === "completed").length;
...
var StatsView = class extends import_obsidian2.ItemView {
  ...
  render() {
    if (!this.root)
      return;
    this.root.render(
      /* @__PURE__ */ React.createElement(StatsComponent, { plugin: this.plugin })
    );
  }
};
var MarkdownBlock = ({ plugin, markdown, sourcePath }) => {
  ...
  React.useEffect(() => {
    ...
    void import_obsidian2.MarkdownRenderer.render(plugin.app, markdown, el, sourcePath, component);
    return () => {
      component.unload();
    };
...
var StatsComponent = ({ plugin }) => {
  ...
  const [expandedHighlightPaths, setExpandedHighlightPaths] = React.useState({});
  const [highlightContentByPath, setHighlightContentByPath] = React.useState({});
  const [creatingInboxPath, setCreatingInboxPath] = React.useState(null);
  ...
  const parseHighlightNote = React.useCallback((text) => {
  ...
  const formatRemaining = React.useCallback((minutesRaw) => {
  ...
  const getRemainingMinutes = React.useCallback((book) => {
    const wpm = 200;
  ...
  const minutesFromDay = React.useCallback((day, totalWords) => {
    if (!day)
      return 0;
    if ((day.minutes || 0) > 0)
  ...
  const normalizeName = React.useCallback(
    (s) => s.toLowerCase().replace(/ё/g, "\u0435").replace(/[^a-z0-9а-я]+/gi, " ").trim().replace(/\s+/g, " "),
    []
  );
  const booksFiles = React.useMemo(() => {
    var _a;
    const booksRaw = ((_a = plugin.settings) == null ? void 0 : _a.readwiseBooksFolder) || "Readwise/Books";
    const booksRoot = (0, import_obsidian2.normalizePath)(String(booksRaw)).replace(/^\/+/, "").replace(/\/+$/, "");
  ...
  const bookNoteFile = React.useMemo(() => {
    ...
    for (const f of booksFiles) {
      ...
      const fmUrl = typeof (fm == null ? void 0 : fm.url) === "string" ? fm.url : "";
    ...
    for (const f of booksFiles) {
      const cache = plugin.app.metadataCache.getFileCache(f);
      const fm = cache == null ? void 0 : cache.frontmatter;
      const fmTitle = typeof (fm == null ? void 0 : fm.title) === "string" ? fm.title : "";
  ...
  const highlightsFiles = React.useMemo(() => {
    ...
    if (!activeBook)
      return [];
    const rootRaw = ((_a = plugin.settings) == null ? void 0 : _a.readwiseLinkedHighlightsFolder) || "Readwise/Highlights";
    const root = (0, import_obsidian2.normalizePath)(String(rootRaw)).replace(/^\/+/, "").replace(/\/+$/, "");
    const id = String(activeBook.readwise_id || activeBook.id || "");
    let folderName = null;
    for (const f of booksFiles) {
      const cache = plugin.app.metadataCache.getFileCache(f);
    ...
    if (!folderName) {
      const wanted2 = normalizeName(activeBook.title);
      for (const f of booksFiles) {
        const cache = plugin.app.metadataCache.getFileCache(f);
        const fm = cache == null ? void 0 : cache.frontmatter;
        const fmTitle = typeof (fm == null ? void 0 : fm.title) === "string" ? fm.title : "";
        if (normalizeName(fmTitle) === wanted2 || normalizeName(f.basename) === wanted2) {
          folderName = f.basename;
          break;
        }
      }
    ...
    const wanted = normalizeName(activeBook.title);
    return plugin.app.vault.getMarkdownFiles().filter((f) => root ? f.path.startsWith(rootPrefix) : false).filter((f) => {
      var _a2;
      const cache = plugin.app.metadataCache.getFileCache(f);
      const bookField = (_a2 = cache == null ? void 0 : cache.frontmatter) == null ? void 0 : _a2.book;
  ...
  const bookMatches = React.useMemo(() => {
    const q = normalizeName(bookQuery);
    const list = books.slice().sort((a, b) => a.title.localeCompare(b.title, "ru"));
    if (!q)
    ...
    const scored = list.map((b) => {
      ...
      return { b, score };
    }).filter((x) => Number.isFinite(x.score)).sort((a, b) => a.score - b.score || a.b.title.localeCompare(b.b.title, "ru"));
    return scored.slice(0, 20).map((x) => x.b);
  }, [bookQuery, books, normalizeName]);
  return /* @__PURE__ */ React.createElement("div", { className: "p-4" }, /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 12 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement("div", { style: { position: "relative", flex: "1 1 320px", minWidth: 240 } }, /* @__PURE__ */ React.createElement(
    "input",
    {
      value: bookQuery,
      onChange: (e) => {
  ...
  ), bookPickerOpen && bookMatches.length > 0 ? /* @__PURE__ */ React.createElement(
    "div",
    {
      style: {
        position: "absolute",
    ...
    bookMatches.map((b) => /* @__PURE__ */ React.createElement(
      "div",
      {
        key: b.id,
      ...
      /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, fontWeight: 600, lineHeight: 1.25 } }, b.title),
      b.author ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, opacity: 0.75, marginTop: 2 } }, b.author) : null
    ))
    ...
    "\u0427\u0438\u0442\u0430\u044E \u0441\u0435\u0439\u0447\u0430\u0441"
  )), !activeBook ? /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, fontSize: 12, opacity: 0.75 } }, "\u0412\u044B\u0431\u0435\u0440\u0438\u0442\u0435 \u043A\u043D\u0438\u0433\u0443, \u0447\u0442\u043E\u0431\u044B \u043F\u043E\u0441\u043C\u043E\u0442\u0440\u0435\u0442\u044C \u0435\u0451 \u0437\u0430\u043C\u0435\u0442\u043A\u0438.") : null), !activeBook ? null : /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement(
    "div",
    {
      style: {
        display: "flex",
    ...
    /* @__PURE__ */ React.createElement("div", { style: { width: 46, height: 64, flex: "0 0 auto", borderRadius: 6, overflow: "hidden", background: "rgba(27,31,35,0.06)" } }, activeBook.cover_url ? /* @__PURE__ */ React.createElement(
      "img",
      {
    ...
    /* @__PURE__ */ React.createElement("div", { style: { flex: "1 1 auto", minWidth: 240 } }, /* @__PURE__ */ React.createElement("div", { style: { fontWeight: 700, fontSize: 14, lineHeight: 1.2 } }, activeBook.title), /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, opacity: 0.75, marginTop: 3 } }, activeBook.author || ""), activeBook.tags && activeBook.tags.length > 0 ? /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 6, marginTop: 8 } }, activeBook.tags.slice(0, 20).map((t) => /* @__PURE__ */ React.createElement(
      "span",
      {
        key: t,
        style: {
      ...
      t
    ))) : null, /* @__PURE__ */ React.createElement("div", { style: { marginTop: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", gap: 12, fontSize: 12, opacity: 0.85 } }, /* @__PURE__ */ React.createElement("div", null, Math.min(100, Math.max(0, activeBook.reading_progress || 0)).toFixed(1), "%"), /* @__PURE__ */ React.createElement("div", { style: { whiteSpace: "nowrap" } }, "\u0443\u0448\u043B\u043E: ", formatRemaining(spentMinutes), " \xB7 ", "\u043E\u0441\u0442\u0430\u043B\u043E\u0441\u044C: ", (() => {
      const remaining = getRemainingMinutes(activeBook);
      return remaining === null ? "\u2014" : formatRemaining(remaining);
    })())), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 6, height: 6, borderRadius: 999, background: "rgba(27,31,35,0.12)", overflow: "hidden" } }, /* @__PURE__ */ React.createElement("div", { style: { width: `${Math.min(100, Math.max(0, activeBook.reading_progress || 0))}%`, height: "100%", background: "#3b82f6" } }))))
  ), /* @__PURE__ */ React.createElement("div", { style: { marginTop: 18 } }, /* @__PURE__ */ React.createElement("h2", { style: { fontSize: 18, fontWeight: 700, margin: 0, marginBottom: 10 } }, "Highlights"), highlightItems.length === 0 ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 13, opacity: 0.75 } }, "\u041D\u0435\u0442 \u0441\u043E\u0437\u0434\u0430\u043D\u043D\u044B\u0445 \u0445\u0430\u0439\u043B\u0430\u0439\u0442\u043E\u0432 \u0432 \u043F\u0430\u043F\u043A\u0435 \u0441\u0432\u044F\u0437\u043E\u043A.") : /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap", marginBottom: 10 } }, /* @__PURE__ */ React.createElement(
    "button",
    {
      onClick: () => {
        var _a;
        (_a = plugin.openBookGraph) == null ? void 0 : _a.call(plugin, activeBook.id);
      },
      style: {
        fontSize: 12,
        padding: "6px 10px",
    ...
    "\u041E\u0442\u043A\u0440\u044B\u0442\u044C \u0433\u0440\u0430\u0444"
  ), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 6, flexWrap: "wrap" } }, /* @__PURE__ */ React.createElement(
    "button",
    {
  ...
  ))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 8 } }, highlightItems.slice(0, 300).map((h) => {
    ...
    return /* @__PURE__ */ React.createElement(
      ...
      /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8, minWidth: 0, flex: "1 1 auto" } }, /* @__PURE__ */ React.createElement(
        "button",
        {
          onClick: async () => {
      ...
      )), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6, flex: "0 0 auto" } }, /* @__PURE__ */ React.createElement(
        "button",
        {
          onClick: async () => {
            try {
      ...
      h.date ? /* @__PURE__ */ React.createElement("div", { style: { fontSize: 12, opacity: 0.7, marginTop: 4 } }, formatDate(h.date)) : null,
      isExpanded ? /* @__PURE__ */ React.createElement("div", { style: { marginTop: 8, fontSize: 12, lineHeight: 1.45, opacity: 0.9 } }, !cached ? /* @__PURE__ */ React.createElement("div", { style: { opacity: 0.7 } }, "\u0417\u0430\u0433\u0440\u0443\u0437\u043A\u0430\u2026") : /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement(
        MarkdownBlock,
        {
          plugin,
      ...
      ), !cached.quote && !cached.description ? /* @__PURE__ */ React.createElement("div", { style: { opacity: 0.7 } }, "\u041F\u0443\u0441\u0442\u0430\u044F \u0437\u0430\u043C\u0435\u0442\u043A\u0430.") : null)) : null
    );
  }))))));
};
...
var DashboardView = class extends import_obsidian3.ItemView {
  ...
  render() {
    if (!this.root)
      return;
    this.root.render(
      /* @__PURE__ */ React2.createElement(DashboardComponent, { plugin: this.plugin })
    );
  }
};
var DashboardComponent = ({ plugin }) => {
  const [books, setBooks] = React2.useState([]);
  const [readingActivity, setReadingActivity] = React2.useState({});
  const [readingActivityByBook, setReadingActivityByBook] = React2.useState({});
  const [selectedBookId, setSelectedBookId] = React2.useState(null);
  const [completedCollapsed, setCompletedCollapsed] = React2.useState(true);
  const [selectedTags, setSelectedTags] = React2.useState([]);
  const [selectedDateKey, setSelectedDateKey] = React2.useState(null);
  const loadData = React2.useCallback(() => {
    const data = plugin.dataManager.getData();
    setBooks(Object.values(data.books));
  ...
  const toDateKey = React2.useCallback((iso) => {
    const d = new Date(iso);
  ...
  const selectedBook = React2.useMemo(() => {
    if (!selectedBookId)
      return null;
    return books.find((b) => b.id === selectedBookId) || null;
  }, [books, selectedBookId]);
  const allTags = React2.useMemo(() => {
  ...
  const isBookMatchingTags = React2.useCallback((book) => {
  ...
  const filteredBooks = React2.useMemo(() => {
    if (selectedTags.length === 0)
      return books;
  ...
  const dayHasActivity = React2.useCallback((day) => {
    if (!day)
      return false;
    if ((day.minutes || 0) > 0.01)
      return true;
    if ((day.words || 0) > 0.01)
      return true;
    if ((day.progressPoints || 0) > 0.01)
      return true;
    if ((day.events || 0) > 0)
      return true;
    return false;
  }, []);
  const activeBookIdsForSelectedDate = React2.useMemo(() => {
    if (!selectedDateKey)
      return null;
    const s = /* @__PURE__ */ new Set();
    for (const book of filteredBooks) {
      ...
      if (iso && toDateKey(iso) === selectedDateKey)
        s.add(book.id);
    }
    return s;
  }, [dayHasActivity, filteredBooks, readingActivityByBook, selectedDateKey, toDateKey]);
  const fallbackUpdatesByDate = React2.useMemo(() => {
    const counts = {};
    const sourceBooks = selectedBookId ? books.filter((b) => b.id === selectedBookId) : filteredBooks;
  ...
  const activitySource = React2.useMemo(() => {
    if (selectedBookId)
  ...
  const getEstimatedTotalWords = React2.useCallback((book) => {
    if (!book)
      return 0;
  ...
  const getMinutesForDay = React2.useCallback((day) => {
    if (!day)
      return 0;
    if ((day.minutes || 0) > 0)
      return day.minutes || 0;
  ...
  const heatmapMode = React2.useMemo(() => {
    const values = Object.values(activitySource);
    if (values.some((v) => (v.minutes || 0) > 0.01 || (v.words || 0) > 0.01))
      return "minutes";
    const selectedBookHasEstimatedWords = selectedBookId ? getEstimatedTotalWords(selectedBook) > 0 : false;
    if (selectedBookHasEstimatedWords && values.some((v) => (v.progressPoints || 0) > 0.01))
      return "minutes";
    if (values.some((v) => (v.progressPoints || 0) > 0.01))
      return "progressPoints";
    return "updates";
  }, [activitySource, getEstimatedTotalWords, selectedBook, selectedBookId]);
  const heatmapValueByDate = React2.useCallback((dateKey) => {
    var _a;
    if (heatmapMode === "minutes")
      return getMinutesForDay(activitySource[dateKey]);
  ...
  const { weeks, rangeStart, rangeEnd, maxValue, totalValue, avgValue, activeDays, legendLabel } = React2.useMemo(() => {
    ...
    const avgValue2 = activeDays2 > 0 ? totalValue2 / activeDays2 : 0;
    const legendLabel2 = heatmapMode === "minutes" ? "\u041C\u0438\u043D\u0443\u0442\u044B \u0447\u0442\u0435\u043D\u0438\u044F" : heatmapMode === "progressPoints" ? "\u041F\u0440\u043E\u0433\u0440\u0435\u0441\u0441 (\u043F.\u043F.)" : "\u041A\u043D\u0438\u0433\u0438 (\u043E\u0431\u043D\u043E\u0432\u043B\u0435\u043D\u0438\u044F)";
    return { weeks: weeks2, rangeStart: start, rangeEnd: end, maxValue: maxValue2, totalValue: totalValue2, avgValue: avgValue2, activeDays: activeDays2, legendLabel: legendLabel2 };
  }, [heatmapMode, heatmapValueByDate]);
  const selectedDayValue = React2.useMemo(() => {
    if (!selectedDateKey)
      return null;
    return heatmapValueByDate(selectedDateKey);
  }, [heatmapValueByDate, selectedDateKey]);
  const selectedDayLabel = React2.useMemo(() => {
    if (!selectedDateKey)
      return null;
  ...
  const statsPanel = React2.useMemo(() => {
    if (!selectedDateKey) {
      return {
  ...
  const heatmapLevel = React2.useCallback((value) => {
    if (value <= 0)
      return 0;
    if (heatmapMode === "minutes") {
      if (value < 10)
  ...
  const heatmapValueFormat = React2.useCallback((value) => {
    if (heatmapMode === "minutes")
  ...
  const heatmapViewportRef = React2.useRef(null);
  const [visibleWeekCount, setVisibleWeekCount] = React2.useState(weeks.length);
  React2.useEffect(() => {
  ...
  const displayedWeeks = React2.useMemo(() => {
  ...
  const monthLabelByWeekIndex = React2.useMemo(() => {
    const monthText = (d) => new Intl.DateTimeFormat("ru-RU", { month: "short" }).format(d).replace(".", "");
    ...
    for (let wi = 0; wi < displayedWeeks.length; wi++) {
      ...
      const firstInRange = week.find((d) => d >= rangeStart && d <= rangeEnd) || week.find((d) => d <= rangeEnd) || week[0];
  ...
  const readingBooks = React2.useMemo(() => {
    return filteredBooks.filter((b) => b.status === "reading").filter((b) => activeBookIdsForSelectedDate ? activeBookIdsForSelectedDate.has(b.id) : true).sort((a, b) => new Date(b.updated_at || b.created_at).getTime() - new Date(a.updated_at || a.created_at).getTime());
  }, [activeBookIdsForSelectedDate, filteredBooks]);
  const completedBooks = React2.useMemo(() => {
    return filteredBooks.filter((b) => b.status === "completed").filter((b) => activeBookIdsForSelectedDate ? activeBookIdsForSelectedDate.has(b.id) : true).sort((a, b) => new Date(b.updated_at || b.created_at).getTime() - new Date(a.updated_at || a.created_at).getTime());
  }, [activeBookIdsForSelectedDate, filteredBooks]);
  const formatRemaining = React2.useCallback((minutesRaw) => {
    const minutes = Math.max(0, Math.round(minutesRaw));
    const h = Math.floor(minutes / 60);
    const m = minutes % 60;
    if (h <= 0)
  ...
  const getRemainingMinutes = React2.useCallback((book) => {
    const wpm = 200;
    const totalWords = book.words_count || 0;
    if (totalWords <= 0)
      return null;
  ...
  const formatDate = React2.useCallback((iso) => {
    if (!iso)
      return "";
    const d = new Date(iso);
    if (Number.isNaN(d.getTime()))
  ...
  return /* @__PURE__ */ React2.createElement("div", { className: "p-4" }, allTags.length > 0 ? /* @__PURE__ */ React2.createElement("div", { style: { marginBottom: 10, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" } }, /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 6 } }, allTags.slice(0, 40).map((t) => {
  ...
  })), selectedTags.length > 0 ? /* @__PURE__ */ React2.createElement(
    "button",
    {
    ...
    "\u0421\u0431\u0440\u043E\u0441\u0438\u0442\u044C \u0442\u0435\u0433\u0438"
  ) : null) : null, /* @__PURE__ */ React2.createElement("div", null, /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" } }, /* @__PURE__ */ React2.createElement(
    "div",
    {
      ref: heatmapViewportRef,
      style: { overflow: "hidden", paddingBottom: 4, maxWidth: "100%", flex: "1 1 0", minWidth: 0 }
    },
    /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 6 } }, showMonthRow ? /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", gap: heatmapCellGap, paddingLeft: heatmapDayLabelWidth + heatmapLabelGap, overflow: "hidden" } }, displayedWeeks.map((_, wi) => {
      const label = monthLabelByWeekIndex[wi];
      return /* @__PURE__ */ React2.createElement("div", { key: wi, style: { width: heatmapCellSize, height: 14, position: "relative" } }, label ? /* @__PURE__ */ React2.createElement("div", { style: { position: "absolute", left: 0, top: 0, fontSize: 12, opacity: 0.8, whiteSpace: "nowrap" } }, label) : null);
    })) : null, /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", alignItems: "flex-start", gap: heatmapLabelGap } }, /* @__PURE__ */ React2.createElement("div", { style: { width: heatmapDayLabelWidth, display: "flex", flexDirection: "column", gap: heatmapCellGap } }, Array.from({ length: 7 }).map((_, di) => {
    ...
    })), /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", gap: heatmapCellGap } }, displayedWeeks.map((week, wi) => /* @__PURE__ */ React2.createElement("div", { key: wi, style: { display: "flex", flexDirection: "column", gap: heatmapCellGap } }, week.map((d, di) => {
  ...
  ), /* @__PURE__ */ React2.createElement("div", { style: { minWidth: 220 } }, /* @__PURE__ */ React2.createElement("div", { style: { fontSize: 12, opacity: 0.8, marginBottom: 8 } }, legendLabel), /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", alignItems: "center", gap: 6, marginBottom: 10 } }, /* @__PURE__ */ React2.createElement("span", { style: { fontSize: 12, opacity: 0.8 } }, "\u041C\u0435\u043D\u044C\u0448\u0435"), /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", gap: 3 } }, heatmapColors.map((c, idx) => /* @__PURE__ */ React2.createElement(
    "div",
    {
      key: idx,
      style: {
  ...
  ))), /* @__PURE__ */ React2.createElement("span", { style: { fontSize: 12, opacity: 0.8 } }, "\u0411\u043E\u043B\u044C\u0448\u0435")), /* @__PURE__ */ React2.createElement("div", { style: { fontSize: 12, opacity: 0.85 } }, /* @__PURE__ */ React2.createElement("div", null, statsPanel.periodLabel, ": ", heatmapValueFormat(statsPanel.total)), /* @__PURE__ */ React2.createElement("div", null, "\u041C\u0430\u043A\u0441. \u0437\u0430 \u0434\u0435\u043D\u044C: ", heatmapValueFormat(statsPanel.max)), /* @__PURE__ */ React2.createElement("div", null, "\u0421\u0440\u0435\u0434\u043D\u0435\u0435 (\u0430\u043A\u0442\u0438\u0432\u043D\u044B\u0439 \u0434\u0435\u043D\u044C): ", heatmapValueFormat(statsPanel.avg)), /* @__PURE__ */ React2.createElement("div", null, "\u0414\u043D\u0435\u0439 \u0441 \u0447\u0442\u0435\u043D\u0438\u0435\u043C: ", statsPanel.active))))), /* @__PURE__ */ React2.createElement("div", { style: { marginTop: 18 } }, /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 12, flexWrap: "wrap" } }, /* @__PURE__ */ React2.createElement("h2", { style: { fontSize: 18, fontWeight: 700, marginBottom: 10 } }, "\u0421\u0435\u0439\u0447\u0430\u0441 \u0447\u0438\u0442\u0430\u044E"), selectedBookId ? /* @__PURE__ */ React2.createElement(
    "button",
    {
    ...
    "\u041F\u043E\u043A\u0430\u0437\u0430\u0442\u044C \u0432\u0441\u0435"
  ) : null), readingBooks.length === 0 ? /* @__PURE__ */ React2.createElement("div", { style: { fontSize: 13, opacity: 0.75 } }, "\u041D\u0435\u0442 \u0430\u043A\u0442\u0438\u0432\u043D\u044B\u0445 \u043A\u043D\u0438\u0433.") : /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 10 } }, readingBooks.map((book) => {
    ...
    return /* @__PURE__ */ React2.createElement(
      ...
      /* @__PURE__ */ React2.createElement("div", { style: { width: 46, height: 64, flex: "0 0 auto", borderRadius: 6, overflow: "hidden", background: "rgba(27,31,35,0.06)" } }, book.cover_url ? /* @__PURE__ */ React2.createElement(
        "img",
        {
          src: book.cover_url,
          alt: book.title,
      ...
      /* @__PURE__ */ React2.createElement("div", { style: { flex: "1 1 auto", minWidth: 240 } }, /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", justifyContent: "space-between", gap: 12, alignItems: "baseline" } }, /* @__PURE__ */ React2.createElement("div", { style: { fontWeight: 700, fontSize: 14, lineHeight: 1.2 } }, book.title), /* @__PURE__ */ React2.createElement("div", { style: { fontSize: 12, opacity: 0.7, whiteSpace: "nowrap" } }, rightDate)), /* @__PURE__ */ React2.createElement("div", { style: { fontSize: 12, opacity: 0.75, marginTop: 3 } }, book.author || ""), /* @__PURE__ */ React2.createElement("div", { style: { marginTop: 8 } }, /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", justifyContent: "space-between", gap: 12, fontSize: 12, opacity: 0.85 } }, /* @__PURE__ */ React2.createElement("div", null, progress.toFixed(1), "%"), /* @__PURE__ */ React2.createElement("div", { style: { whiteSpace: "nowrap" } }, remainingMinutes === null ? "\u043E\u0441\u0442\u0430\u043B\u043E\u0441\u044C: \u2014" : formatRemaining(remainingMinutes))), /* @__PURE__ */ React2.createElement("div", { style: { marginTop: 6, height: 6, borderRadius: 999, background: "rgba(27,31,35,0.12)", overflow: "hidden" } }, /* @__PURE__ */ React2.createElement("div", { style: { width: `${progress}%`, height: "100%", background: "#3b82f6" } }))))
    );
  }))), /* @__PURE__ */ React2.createElement("div", { style: { marginTop: 14 } }, /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 12, flexWrap: "wrap" } }, /* @__PURE__ */ React2.createElement(
    "div",
    {
      role: "button",
      tabIndex: 0,
    ...
    /* @__PURE__ */ React2.createElement("span", null, completedCollapsed ? "\u25B6" : "\u25BC"),
    /* @__PURE__ */ React2.createElement("span", null, "\u041F\u0440\u043E\u0447\u0438\u0442\u0430\u043D\u043E"),
    /* @__PURE__ */ React2.createElement("span", { style: { fontSize: 12, opacity: 0.7 } }, "(", completedBooks.length, ")")
  ), selectedBookId && (selectedBook == null ? void 0 : selectedBook.status) === "completed" ? /* @__PURE__ */ React2.createElement(
    "button",
    {
      onClick: () => setSelectedBookId(null),
      style: {
    ...
    "\u041F\u043E\u043A\u0430\u0437\u0430\u0442\u044C \u0432\u0441\u0435"
  ) : null), completedCollapsed ? null : completedBooks.length === 0 ? /* @__PURE__ */ React2.createElement("div", { style: { fontSize: 13, opacity: 0.75, marginTop: 8 } }, "\u041F\u043E\u043A\u0430 \u043D\u0435\u0442 \u043F\u0440\u043E\u0447\u0438\u0442\u0430\u043D\u043D\u044B\u0445 \u043A\u043D\u0438\u0433.") : /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 10, marginTop: 10 } }, completedBooks.slice(0, 50).map((book) => {
    ...
    return /* @__PURE__ */ React2.createElement(
      ...
      /* @__PURE__ */ React2.createElement("div", { style: { flex: "1 1 auto", minWidth: 240 } }, /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", justifyContent: "space-between", gap: 12, alignItems: "baseline" } }, /* @__PURE__ */ React2.createElement("div", { style: { fontWeight: 700, fontSize: 14, lineHeight: 1.2 } }, book.title), /* @__PURE__ */ React2.createElement("div", { style: { fontSize: 12, opacity: 0.7, whiteSpace: "nowrap" } }, rightDate)), /* @__PURE__ */ React2.createElement("div", { style: { fontSize: 12, opacity: 0.75, marginTop: 3 } }, book.author || ""), /* @__PURE__ */ React2.createElement("div", { style: { marginTop: 8 } }, /* @__PURE__ */ React2.createElement("div", { style: { display: "flex", justifyContent: "space-between", gap: 12, fontSize: 12, opacity: 0.85 } }, /* @__PURE__ */ React2.createElement("div", null, progress.toFixed(1), "%"), /* @__PURE__ */ React2.createElement("div", { style: { whiteSpace: "nowrap" } }, "\u043F\u0440\u043E\u0447\u0438\u0442\u0430\u043D\u043E")), /* @__PURE__ */ React2.createElement("div", { style: { marginTop: 6, height: 6, borderRadius: 999, background: "rgba(27,31,35,0.12)", overflow: "hidden" } }, /* @__PURE__ */ React2.createElement("div", { style: { width: `${progress}%`, height: "100%", background: "#22c55e" } }))))
    );
  }))));
};
...
var BookGraphView = class extends import_obsidian4.ItemView {
  constructor(leaf, plugin) {
    super(leaf);
    this.root = null;
    this.state = {};
  ...
  render() {
    ...
    this.root.render(/* @__PURE__ */ React3.createElement(BookGraphComponent, { plugin: this.plugin, bookId }));
  }
};
var BookGraphComponent = ({ plugin, bookId }) => {
  const [books, setBooks] = React3.useState([]);
  const [highlightsSort, setHighlightsSort] = React3.useState("date");
  const [highlightsSortDir, setHighlightsSortDir] = React3.useState("asc");
  const [scale, setScale] = React3.useState(1);
  const [pan, setPan] = React3.useState({ x: 0, y: 0 });
  const [activeNodeId, setActiveNodeId] = React3.useState(null);
  const [hoveredNodeId, setHoveredNodeId] = React3.useState(null);
  const [tooltipPos, setTooltipPos] = React3.useState(null);
  const [tooltipData, setTooltipData] = React3.useState(null);
  const graphContainerRef = React3.useRef(null);
  const loadData = React3.useCallback(() => {
    const data = plugin.dataManager.getData();
    setBooks(Object.values(data.books));
  }, [plugin.dataManager]);
  React3.useEffect(() => {
  ...
  const currentBook = React3.useMemo(() => {
    if (bookId)
      return books.find((b) => b.id === bookId) || null;
    const reading = books.filter((b) => b.status === "reading").sort((a, b) => new Date(b.updated_at || b.created_at).getTime() - new Date(a.updated_at || a.created_at).getTime());
    return reading[0] || null;
  }, [bookId, books]);
  const normalizeName = React3.useCallback(
    (s) => s.toLowerCase().replace(/ё/g, "\u0435").replace(/[^a-z0-9а-я]+/gi, " ").trim().replace(/\s+/g, " "),
    []
  );
  const booksFiles = React3.useMemo(() => {
    var _a;
    const booksRaw = ((_a = plugin.settings) == null ? void 0 : _a.readwiseBooksFolder) || "Readwise/Books";
    const booksRoot = (0, import_obsidian4.normalizePath)(String(booksRaw)).replace(/^\/+/, "").replace(/\/+$/, "");
    const booksPrefix = booksRoot ? `${booksRoot}/` : "";
    return plugin.app.vault.getMarkdownFiles().filter((f) => booksRoot ? f.path === booksRoot || f.path.startsWith(booksPrefix) : false);
  }, [plugin.app.vault, plugin.settings]);
  const bookNoteFile = React3.useMemo(() => {
    if (!currentBook)
      return null;
  ...
  const highlightsFiles = React3.useMemo(() => {
    var _a, _b;
    if (!currentBook)
      return [];
    const rootRaw = ((_a = plugin.settings) == null ? void 0 : _a.readwiseLinkedHighlightsFolder) || "Readwise/Highlights";
    const root = (0, import_obsidian4.normalizePath)(String(rootRaw)).replace(/^\/+/, "").replace(/\/+$/, "");
    const booksRaw = ((_b = plugin.settings) == null ? void 0 : _b.readwiseBooksFolder) || "Readwise/Books";
    const booksRoot = (0, import_obsidian4.normalizePath)(String(booksRaw)).replace(/^\/+/, "").replace(/\/+$/, "");
    const booksPrefix = booksRoot ? `${booksRoot}/` : "";
    const id = String(currentBook.readwise_id || currentBook.id || "");
    ...
    if (!folderName) {
      const wanted2 = normalizeName(currentBook.title);
      for (const f of booksFilesLocal) {
        const cache = plugin.app.metadataCache.getFileCache(f);
        const fm = cache == null ? void 0 : cache.frontmatter;
        const fmTitle = typeof (fm == null ? void 0 : fm.title) === "string" ? fm.title : "";
    ...
    const folder = (0, import_obsidian4.normalizePath)(`${root}/${preferredFolderName}`);
    const prefix = `${folder}/`;
    const inPreferredFolder = plugin.app.vault.getMarkdownFiles().filter((f) => f.path.startsWith(prefix)).sort((a, b) => a.basename.localeCompare(b.basename, "ru"));
    if (inPreferredFolder.length > 0)
      return inPreferredFolder;
  ...
  const highlightItems = React3.useMemo(() => {
  ...
  const sortedHighlights = React3.useMemo(() => {
    const dir = highlightsSortDir === "asc" ? 1 : -1;
  ...
  const dragRef = React3.useRef({ dragging: false, lastX: 0, lastY: 0 });
  const onWheel = React3.useCallback((e) => {
    e.preventDefault();
    const delta = -e.deltaY;
    const factor = delta > 0 ? 1.08 : 0.92;
    setScale((s) => Math.max(0.2, Math.min(2.5, s * factor)));
  }, []);
  const onMouseDown = React3.useCallback((e) => {
    dragRef.current.dragging = true;
    dragRef.current.lastX = e.clientX;
    dragRef.current.lastY = e.clientY;
  }, []);
  const onMouseMove = React3.useCallback((e) => {
  ...
  const onMouseUp = React3.useCallback(() => {
    dragRef.current.dragging = false;
  }, []);
  const openFile = React3.useCallback((file) => {
    if (!file)
      return;
    plugin.app.workspace.getLeaf(false).openFile(file);
  }, [plugin.app.workspace]);
  const nodeSize = React3.useCallback((kind) => {
    if (kind === "book")
      return { w: 240, h: 86 };
    return { w: 260, h: 80 };
  }, []);
  const intersectRectFromCenter = React3.useCallback((from, to) => {
  ...
  const parseHighlightNote = React3.useCallback((text) => {
    const lines = text.split(/\r?\n/);
    let i = 0;
    if (lines[i] === "---") {
      i++;
  ...
  return /* @__PURE__ */ React3.createElement("div", { className: "p-4", style: { height: "100%", display: "flex", flexDirection: "column", gap: 12 } }, /* @__PURE__ */ React3.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" } }, /* @__PURE__ */ React3.createElement("div", { style: { display: "flex", alignItems: "baseline", gap: 10, flexWrap: "wrap" } }, /* @__PURE__ */ React3.createElement("h1", { style: { fontSize: 18, fontWeight: 700, margin: 0 } }, "\u0413\u0440\u0430\u0444 \u043A\u043D\u0438\u0433\u0438"), /* @__PURE__ */ React3.createElement("div", { style: { fontSize: 12, opacity: 0.75 } }, currentBook ? currentBook.title : "\u041D\u0435\u0442 \u0432\u044B\u0431\u0440\u0430\u043D\u043D\u043E\u0439 \u043A\u043D\u0438\u0433\u0438")), /* @__PURE__ */ React3.createElement("div", { style: { display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" } }, /* @__PURE__ */ React3.createElement(
    "button",
    {
      onClick: () => setHighlightsSort("date"),
      style: {
    ...
    "\u0421\u0431\u0440\u043E\u0441"
  ))), !currentBook ? /* @__PURE__ */ React3.createElement("div", { style: { fontSize: 13, opacity: 0.75 } }, "\u041D\u0435\u0442 \u0442\u0435\u043A\u0443\u0449\u0435\u0439 \u043A\u043D\u0438\u0433\u0438 (status=reading) \u0438 \u043D\u0435 \u043F\u0435\u0440\u0435\u0434\u0430\u043D bookId.") : /* @__PURE__ */ React3.createElement(
    "div",
    {
    ...
    /* @__PURE__ */ React3.createElement(
      ...
      /* @__PURE__ */ React3.createElement("g", { transform: `translate(${pan.x} ${pan.y}) scale(${scale})` }, graph.edges.map((e) => {
        ...
        return /* @__PURE__ */ React3.createElement(
          ...
          /* @__PURE__ */ React3.createElement("rect", { width: w, height: h, rx: 12, ry: 12, fill, stroke, strokeWidth: active ? 2 : 1 }),
          /* @__PURE__ */ React3.createElement("text", { x: 12, y: 26, fontSize: 13, fontWeight: 700, fill: "currentColor" }, n.label.length > 36 ? `${n.label.slice(0, 36)}\u2026` : n.label),
          n.subtitle ? /* @__PURE__ */ React3.createElement("text", { x: 12, y: 46, fontSize: 11, fill: "currentColor", opacity: 0.7 }, n.subtitle.length > 44 ? `${n.subtitle.slice(0, 44)}\u2026` : n.subtitle) : null,
          /* @__PURE__ */ React3.createElement("text", { x: 12, y: 66, fontSize: 11, fill: "currentColor", opacity: 0.7 }, n.kind === "book" ? "\u041A\u043D\u0438\u0433\u0430" : "\u0425\u0430\u0439\u043B\u0430\u0439\u0442")
        );
      }))
    ),
    tooltipPos && tooltipData ? /* @__PURE__ */ React3.createElement(
      "div",
      {
        style: {
          position: "absolute",
      ...
      /* @__PURE__ */ React3.createElement("div", { style: { fontWeight: 700, fontSize: 13, marginBottom: 4, whiteSpace: "pre-wrap" } }, tooltipData.title),
      tooltipData.details ? /* @__PURE__ */ React3.createElement("div", { style: { opacity: 0.8, marginBottom: 8, whiteSpace: "pre-wrap" } }, tooltipData.details) : null,
      tooltipData.quote ? /* @__PURE__ */ React3.createElement("div", { style: { padding: 8, borderRadius: 8, background: "rgba(255,255,255,0.08)", marginBottom: tooltipData.description ? 8 : 0, whiteSpace: "pre-wrap" } }, tooltipData.quote) : null,
      tooltipData.description ? /* @__PURE__ */ React3.createElement("div", { style: { opacity: 0.9, whiteSpace: "pre-wrap" } }, tooltipData.description) : null,
      tooltipData.path ? /* @__PURE__ */ React3.createElement("div", { style: { opacity: 0.6, marginTop: 8, whiteSpace: "pre-wrap" } }, tooltipData.path) : null
    ) : null
  ));
};
...
var ReadwiseTrackerPlugin = class extends import_obsidian5.Plugin {
  async onload() {
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        try {
          ...
          new import_obsidian5.Notice(`Readwise Official sync skipped/failed: ${message}`);
        }
        await this.syncReadwiseData();
        await this.migrateReadwiseBookNotesToLinkedHighlights();
        new import_obsidian5.Notice(officialOk ? "All sync steps completed." : "Tracker sync steps completed.");
      }
    });
    this.addCommand({
  ...
  async openBookGraph(bookId) {
    ...
    if (!leaf)
      return;
    await leaf.setViewState({ type: BOOK_GRAPH_VIEW_TYPE, active: true, state: { bookId } });
    workspace.revealLeaf(leaf);
  }
  findReadwiseOfficialSyncCommandId() {
    var _a, _b;
    const commands = (_b = (_a = this.app) == null ? void 0 : _a.commands) == null ? void 0 : _b.commands;
    if (!commands || typeof commands !== "object")
      return null;
    const entries = Object.entries(commands);
    for (const [id, cmd] of entries) {
      const name = typeof (cmd == null ? void 0 : cmd.name) === "string" ? cmd.name : "";
      const lname = name.toLowerCase();
      if (lname.includes("readwise") && lname.includes("sync") && lname.includes("official"))
        return id;
    }
    for (const [id, cmd] of entries) {
      const name = typeof (cmd == null ? void 0 : cmd.name) === "string" ? cmd.name : "";
      const lname = name.toLowerCase();
      if (lname.includes("readwise") && lname.includes("sync"))
        return id;
      if (id.toLowerCase().includes("readwise") && lname.includes("sync"))
        return id;
    }
    return null;
  }
  async triggerReadwiseOfficialSyncAndWait() {
    const id = this.findReadwiseOfficialSyncCommandId();
    if (!id)
      throw new Error("Readwise Official command not found");
    const normalizeFolder = (v) => (0, import_obsidian5.normalizePath)(String(v || "")).replace(/^\/+/, "").replace(/\/+$/, "");
    ...
    const matches = (path) => prefixes.some((p) => path === p || path.startsWith(`${p}/`));
    ...
    try {
      new import_obsidian5.Notice("Readwise Official: syncing...");
      await Promise.resolve(this.app.commands.executeCommandById(id));
      const startedAt = Date.now();
      const timeoutMs = 5 * 60 * 1e3;
      const idleMs = 2e3;
      const noActivityMaxMs = 15e3;
      while (true) {
        const now = Date.now();
        ...
        if (now - startedAt > timeoutMs)
          throw new Error("Timeout waiting for Readwise Official sync");
  ...
  async createInboxNoteFromHighlight(args) {
    ...
    const inboxFolder = (0, import_obsidian5.normalizePath)(String(inboxRaw)).replace(/^\/+/, "").replace(/\/+$/, "");
    const sanitizeFileName = (s) => s.replace(/[\\/:*?"<>|]/g, " ").replace(/\s+/g, " ").trim().slice(0, 140);
    ...
    const highlightFm = highlightCache == null ? void 0 : highlightCache.frontmatter;
    const highlightTitle = (highlightFm == null ? void 0 : highlightFm.title) || args.highlightFile.basename;
    const highlightDate = (highlightFm == null ? void 0 : highlightFm.date) || "";
    ...
    while (this.app.vault.getAbstractFileByPath(outPath)) {
      outPath = (0, import_obsidian5.normalizePath)(`${inboxFolder}/${baseName} (${n}).md`);
      n++;
    }
    const today = new Date();
    ...
    if (highlightDate)
      contentParts.push(`\u0414\u0430\u0442\u0430: ${highlightDate.slice(0, 10)}`);
    if (parsed.quote) {
      contentParts.push("");
  ...
  async testReadwiseToken() {
    if (!this.settings.readwiseToken) {
      new import_obsidian5.Notice("Please set Readwise API Token in settings.");
      return;
    }
    try {
      await this.readwiseService.validateToken();
      new import_obsidian5.Notice("Readwise token is valid (204).");
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      new import_obsidian5.Notice(`Readwise token test failed: ${message}`);
      console.error(error);
    }
  }
  async syncReadwiseData() {
    ...
    new import_obsidian5.Notice("Syncing Readwise data...");
    try {
      await this.readwiseService.validateToken();
      ...
      for (const book of Object.values(this.dataManager.getData().books)) {
        if (book.source !== "readwise")
          continue;
        if (!filteredIdSet.has(book.id))
      ...
      for (const doc of filteredDocuments) {
        ...
        const tags = doc && typeof doc === "object" && doc.tags && typeof doc.tags === "object" ? Object.keys(doc.tags).filter((t) => typeof t === "string" && t.trim().length > 0) : void 0;
        const book = {
          id: doc.id,
          title,
          author: doc.author || "",
          ...
          status: doc.location === "archive" ? "completed" : (doc.reading_progress || 0) > 0 ? "reading" : "planned"
        };
        if (!existingBook) {
          newCount++;
      ...
      new import_obsidian5.Notice(`Sync complete. Added ${newCount} books, updated ${updateCount} books.`);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      console.error(error);
      new import_obsidian5.Notice(`Failed to sync Readwise data: ${message}`);
    }
  }
  isTopLevelReadingDocument(doc) {
    const category = typeof (doc == null ? void 0 : doc.category) === "string" ? doc.category.toLowerCase() : "";
    if (category === "highlight" || category === "note")
      return false;
    if (doc == null ? void 0 : doc.parent_id)
      return false;
    const title = this.getDocumentTitle(doc);
    if (!title)
      return false;
    if (title.toLowerCase() === "readwise & reader changelog")
      return false;
    return true;
  }
  getDocumentTitle(doc) {
    const candidates = [doc == null ? void 0 : doc.title, doc == null ? void 0 : doc.readable_title, doc == null ? void 0 : doc.site_name];
    ...
    const sourceUrl = typeof (doc == null ? void 0 : doc.source_url) === "string" ? doc.source_url.trim() : "";
    if (sourceUrl) {
      try {
  ...
  parseReadwiseHighlightsFromMarkdown(markdown) {
  ...
  buildLinkedHighlightNoteContent(params) {
    ...
    const quote = params.text.split("\n").map((l) => l.trimEnd()).filter((l) => l.length > 0).map((l) => `> ${l}`).join("\n");
    let content = `---
type: highlight
...
`;
  ...
  async migrateReadwiseBookNotesToLinkedHighlights() {
    const sourceFolder = (0, import_obsidian5.normalizePath)(this.settings.readwiseBooksFolder || "").replace(/^\/+/, "").replace(/\/+$/, "");
    const destRoot = (0, import_obsidian5.normalizePath)(this.settings.readwiseLinkedHighlightsFolder || "").replace(/^\/+/, "").replace(/\/+$/, "");
    if (!sourceFolder || !destRoot) {
      new import_obsidian5.Notice("Please set Readwise folders in settings.");
      return;
    }
    ...
    if (sourceFiles.length === 0) {
      new import_obsidian5.Notice(`No markdown files found in ${sourceFolder}`);
      return;
    }
    new import_obsidian5.Notice(`Migrating Readwise notes: ${sourceFiles.length} files...`);
    let created = 0;
    let skipped = 0;
    let errors = 0;
    for (const file of sourceFiles) {
      try {
        ...
        for (let i = 1; i <= total; i++) {
          ...
          const outPath = (0, import_obsidian5.normalizePath)(`${outDir}/${bookTitle} \u2014 ${String(i).padStart(3, "0")}.md`);
    ...
    new import_obsidian5.Notice(`Migration complete. Created ${created}, skipped ${skipped}, errors ${errors}.`);
  }
};
var ReadwiseTrackerSettingTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian5.Setting(containerEl).setName("Readwise Access Token").setDesc("Your API token from https://readwise.io/access_token").addText((text) => text.setPlaceholder("Enter your token").setValue(this.plugin.settings.readwiseToken).onChange(async (value) => {
      this.plugin.settings.readwiseToken = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian5.Setting(containerEl).setName("Debug logging").setDesc("Log Readwise requests and statuses to the developer console").addToggle((toggle) => toggle.setValue(this.plugin.settings.debugLogging).onChange(async (value) => {
      this.plugin.settings.debugLogging = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian5.Setting(containerEl).setName("Readwise books folder").setDesc("Folder with markdown files created by the Readwise official plugin (books).").addText((text) => text.setPlaceholder("Readwise/Books").setValue(this.plugin.settings.readwiseBooksFolder).onChange(async (value) => {
      this.plugin.settings.readwiseBooksFolder = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian5.Setting(containerEl).setName("Linked highlights folder").setDesc("Destination folder for linked highlight notes (one subfolder per book).").addText((text) => text.setPlaceholder("Readwise/Highlights").setValue(this.plugin.settings.readwiseLinkedHighlightsFolder).onChange(async (value) => {
      this.plugin.settings.readwiseLinkedHighlightsFolder = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian5.Setting(containerEl).setName("Inbox folder").setDesc("Destination folder for notes created from highlights in Readwise Stats.").addText((text) => text.setPlaceholder("Inbox/Readwise").setValue(this.plugin.settings.readwiseInboxFolder).onChange(async (value) => {
      this.plugin.settings.readwiseInboxFolder = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian5.Setting(containerEl).setName("Test Readwise Token").setDesc("Performs GET https://readwise.io/api/v2/auth/ (expects 204)").addButton(
