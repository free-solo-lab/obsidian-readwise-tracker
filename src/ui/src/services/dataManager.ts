...
export class DataManager {
    ...
    public addReadingActivity(
        ...
        const existing = this.data.readingActivity[dateKey] || { minutes: 0, words: 0, progressPoints: 0, events: 0 };
        this.data.readingActivity[dateKey] = {
            minutes: existing.minutes + (delta.minutes || 0),
            words: existing.words + (delta.words || 0),
            progressPoints: existing.progressPoints + (delta.progressPoints || 0),
            events: existing.events + (delta.events || 0),
        };
