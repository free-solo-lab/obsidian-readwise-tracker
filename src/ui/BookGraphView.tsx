...
import { findBookNoteFile, findHighlightFilesForBook } from '../services/readwiseFiles';
import { parseHighlightNote } from '../services/readwiseHighlightParsing';
import { getCurrentLocale, getSortLocale, t } from '../i18n';
...
const BookGraphComponent: React.FC<{ plugin: ReadwiseTrackerViewHost; bookId?: string }> = ({ plugin, bookId }) => {
        ...
        <div className="p-4" style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
                    <h1 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>{t('view.bookGraph')}</h1>
                    <div style={{ fontSize: 12, opacity: 0.75 }}>
                        {currentBook ? currentBook.title : t('graph.noSelectedBook')}
                    </div>
                </div>
            ...
            {!currentBook ? (
                <div style={{ fontSize: 13, opacity: 0.75 }}>{t('graph.noCurrentBook')}</div>
            ) : (
                <div
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <text x={12} y={66} fontSize={11} fill="currentColor" opacity={0.7}>
                                            {n.kind === 'book' ? t('graph.book') : t('graph.highlight')}
                                        </text>
                                    </g>
                                );
                            })}
