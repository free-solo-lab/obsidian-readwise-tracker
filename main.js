...
__export(main_exports, {
  default: () => ReadwiseTrackerPlugin_default
});
...
function translate(locale, key, params = {}) {
  ...
  return template.replace(/\{(\w+)\}/g, (_match, name) => {
    var _a;
    return String((_a = params[name]) != null ? _a : "");
  });
}
function formatDurationCompact(minutesRaw, locale) {
...
function getPlanningStatus(book) {
  if (book.location === "archive")
    return "completed";
  if (book.location === "new")
    return "reading";
  if (book.location === "later" || book.location === "shortlist" || book.location === "feed")
    return "planned";
...
function getReaderLocation(status) {
  if (status === "completed")
    return "archive";
  if (status === "reading")
    return "new";
...
function groupPlanningBooksByTag(books, locale, tagScope = []) {
  ...
  const result = Array.from(groups.entries()).sort(([a], [b]) => a.localeCompare(b, locale)).map(([tag, groupedBooks]) => ({ tag, books: groupedBooks }));
  if (untagged.length > 0)
    result.push({ tag: null, books: untagged });
  return result;
}
    ...
    return /* @__PURE__ */ React6.createElement("section", { key: groupKey, className: `readwise-planning-tag-group${collapsed ? " is-collapsed" : " is-focus"}` }, /* @__PURE__ */ React6.createElement(
      ...
      "div",
      {
        ...
        onDrop: (event) => {
          ...
          if (!book || getPlanningStatus(book) === column.status)
            return;
...
function resolveHeatmapDateRange(range, today) {
  ...
  if (start > selectedEnd)
    return { start: selectedEnd, end: start };
...
function hasBookActivityInRange(book, activityByDate, range) {
  const activityDates = Object.entries(activityByDate || {}).filter(([, activity]) => hasReadingActivity(activity)).map(([dateKey]) => dateKey);
...
var DashboardComponent = ({ plugin }) => {
  ...
  const moveBook = React8.useCallback(async (bookId, status) => {
    ...
    try {
      ...
      loadData();
      new import_obsidian2.Notice(t("dashboard.moveSuccess", { location: t(
        status === "planned" ? "dashboard.readerLater" : status === "reading" ? "dashboard.readerInbox" : "dashboard.readerArchive"
      ) }));
    } catch (error) {
      setBooks((current) => current.map((book) => book.id === bookId ? previous : book));
      new import_obsidian2.Notice(t("dashboard.moveFailed", {
...
// src/services/readwiseFileNames.ts
...
function getReadwiseBookFiles(app, settings) {
  ...
  return app.vault.getMarkdownFiles().filter((file) => booksRoot ? file.path === booksRoot || file.path.startsWith(booksPrefix) : false);
}
function findBookNoteFile(app, settings, book) {
  ...
  for (const file of booksFiles) {
    ...
    const url = typeof (frontmatter == null ? void 0 : frontmatter.url) === "string" ? frontmatter.url : "";
  ...
  for (const file of booksFiles) {
    const cache = app.metadataCache.getFileCache(file);
    const frontmatter = cache == null ? void 0 : cache.frontmatter;
    const title = typeof (frontmatter == null ? void 0 : frontmatter.title) === "string" ? frontmatter.title : "";
    if (normalizeSearchName(title) === wanted || normalizeSearchName(file.basename) === wanted || normalizeCompactName(title) === compactWanted || normalizeCompactName(file.basename) === compactWanted) {
      return file;
    }
  }
  return null;
}
function findHighlightFilesForBook(app, settings, book) {
  ...
  const wanted = normalizeSearchName(book.title);
  const compactWanted = normalizeCompactName(book.title);
  return app.vault.getMarkdownFiles().filter((file) => root ? file.path.startsWith(rootPrefix) : false).filter((file) => {
    const cache = app.metadataCache.getFileCache(file);
    const frontmatter = cache == null ? void 0 : cache.frontmatter;
    const bookField = typeof (frontmatter == null ? void 0 : frontmatter.book) === "string" ? frontmatter.book : "";
    ...
    return normalizeSearchName(file.path).includes(wanted) || normalizeCompactName(file.path).includes(compactWanted);
  }).sort((a, b) => a.basename.localeCompare(b.basename, sortLocale));
}

// src/services/readwiseHighlightParsing.ts
function parseHighlightNote(text) {
...
function parseReadwiseHighlightsFromMarkdown(markdown) {
...
function buildLinkedHighlightNoteContent(params) {
  ...
  const quote = params.text.split("\n").map((line) => line.trimEnd()).filter((line) => line.length > 0).map((line) => `> ${line}`).join("\n");
  let content = `---
type: highlight
...
async function activateReadwiseView(workspace, viewType) {
...
function buildReadwiseInboxNoteContent(input) {
  var _a, _b;
  const parts = [];
  parts.push("---");
  ...
  parts.push("---");
  const quote = (_a = input.quote) == null ? void 0 : _a.trim();
  if (quote) {
    parts.push("");
  ...
  const description = (_b = input.description) == null ? void 0 : _b.trim();
  if (description) {
    parts.push("");
...
var ReadwiseNoteService = class {
  ...
  async createInboxNoteFromHighlight(settings, args) {
    ...
    const highlightTitle = (typeof (frontmatter == null ? void 0 : frontmatter.title) === "string" ? frontmatter.title : void 0) || args.highlightFile.basename;
    const highlightDate = typeof (frontmatter == null ? void 0 : frontmatter.date) === "string" ? frontmatter.date : "";
    ...
    const content = buildReadwiseInboxNoteContent({
      ...
      sourceDate: highlightDate ? highlightDate.slice(0, 10) : void 0,
      quote: parsed.quote,
      description: parsed.description
    });
    const created = await this.app.vault.create(outPath, content);
    await ((_a = this.app.workspace.getLeaf(false)) == null ? void 0 : _a.openFile(created));
    return created;
  }
...
function getHttpErrorStatus(error) {
  ...
  if (typeof error === "object" && error !== null) {
    ...
    const status = (_b = candidate.status) != null ? _b : (_a = candidate.response) == null ? void 0 : _a.status;
...
function getRateLimitWaitMs(source) {
  var _a, _b, _c;
  let headers;
  if (typeof source === "object" && source !== null) {
    const candidate = source;
    headers = (_b = candidate.headers) != null ? _b : (_a = candidate.response) == null ? void 0 : _a.headers;
  }
  const retryAfter = (_c = headers == null ? void 0 : headers["Retry-After"]) != null ? _c : headers == null ? void 0 : headers["retry-after"];
  if (typeof retryAfter === "string") {
    const seconds = Number.parseInt(retryAfter, 10);
    if (Number.isFinite(seconds)) {
      return Math.max(1, seconds) * 1e3;
    }
    const retryAt = Date.parse(retryAfter);
    if (Number.isFinite(retryAt)) {
      return Math.max(1e3, retryAt - Date.now());
    }
  }
...
var ReadwiseService = class {
  ...
  setRequestDelayMs(ms) {
    this.requestDelayMs = Number.isFinite(ms) ? Math.min(6e4, Math.max(0, ms)) : 0;
  }
  setMaxRetries(retries) {
    this.maxRetries = Number.isFinite(retries) ? Math.min(20, Math.max(0, Math.floor(retries))) : 5;
  }
  async validateToken() {
    if (!this.token) {
  ...
  async requestJson(endpoint, options) {
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      let response;
      try {
        response = await (0, import_obsidian6.requestUrl)(requestParams);
      } catch (error) {
...
var ReadwiseOfficialSyncService = class {
  ...
  getCommandRegistry() {
    ...
    return (_a = appWithCommands.commands) != null ? _a : null;
  }
  findOfficialSyncCommandId() {
    const registry = this.getCommandRegistry();
    const commands = registry == null ? void 0 : registry.commands;
    if (!commands || typeof commands !== "object") {
      return null;
    }
    const entries = Object.entries(commands);
    for (const [id, command] of entries) {
      const name = typeof (command == null ? void 0 : command.name) === "string" ? command.name.toLowerCase() : "";
...
// src/services/readwiseDocuments.ts
function getDocumentTitle(doc) {
  const candidates = [doc.title, doc.readable_title, doc.site_name];
...
function isTopLevelReadingDocument(doc) {
...
function isPdfDocument(doc) {
  ...
  const urls = [doc.url, doc.source_url].filter((value) => typeof value === "string" && value.trim().length > 0).map((value) => value.toLowerCase());
...
// src/services/readwisePdfTimeline.ts
function extractHighlightDateKeysFromMarkdown(markdown) {
  try {
    return parseReadwiseHighlightsFromMarkdown(markdown).map((highlight) => highlight.date).filter((date) => typeof date === "string" && date.trim().length > 0).map((date) => toDateKey(date));
  } catch (e) {
    return [];
  }
}
function buildPdfReadingActivity(book, highlightDateKeys) {
...
// src/services/readwisePdfActivity.ts
async function inferPdfReadingActivity(app, settings, book) {
  const bookFile = findBookNoteFile(app, settings, book);
  if (bookFile) {
    const markdown = await app.vault.cachedRead(bookFile);
    const dateKeys2 = extractHighlightDateKeysFromMarkdown(markdown);
    if (dateKeys2.length > 0) {
      return buildPdfReadingActivity(book, dateKeys2);
    }
  }
  const highlightFiles = findHighlightFilesForBook(app, settings, book);
  const dateKeys = highlightFiles.map((file) => {
    const cache = app.metadataCache.getFileCache(file);
    const frontmatter = cache == null ? void 0 : cache.frontmatter;
    return typeof (frontmatter == null ? void 0 : frontmatter.date) === "string" ? frontmatter.date : "";
  }).filter((date) => date.length > 0).map((date) => toDateKey(date));
...
// src/services/readwiseSyncPlan.ts
var INCREMENTAL_SYNC_OVERLAP_MS = 5 * 60 * 1e3;
function createRegularSyncPlan(lastSync, configuredLocations) {
  ...
  const locations = configuredLocations.length > 0 ? configuredLocations : [void 0];
  return {
    mode: "bootstrap",
...
// src/services/ReadwiseSyncService.ts
var ReadwiseSyncService = class {
  constructor(readwiseService, dataManager, app, getSettings) {
    this.readwiseService = readwiseService;
  ...
  async syncFullHistory(debugLogging, options) {
    await this.readwiseService.validateToken();
  ...
  toLocalBook(document2) {
    const tags = document2.tags && typeof document2.tags === "object" ? Object.keys(document2.tags).filter((tag) => typeof tag === "string" && tag.trim().length > 0) : void 0;
    const readingProgress = clampProgress((document2.reading_progress || 0) * 100);
    return {
      id: document2.id,
      title: getDocumentTitle(document2),
      author: document2.author || "",
      category: document2.category || "reader",
      source: "readwise",
      readwise_id: document2.id,
      tags,
      created_at: document2.created_at || new Date().toISOString(),
      updated_at: document2.updated_at || new Date().toISOString(),
      cover_url: document2.image_url,
      reading_progress: readingProgress,
      words_count: document2.word_count,
      notes_count: document2.num_highlights || 0,
      location: document2.location,
  ...
  display() {
    ...
    new import_obsidian9.Setting(containerEl).setName(t("settings.requestDelayName")).setDesc(t("settings.requestDelayDesc")).addText(
      (text) => text.setValue(String(this.plugin.settings.requestDelayMs)).onChange(async (value) => {
        ...
        this.plugin.settings.requestDelayMs = Number.isFinite(parsed) ? Math.min(6e4, Math.max(0, parsed)) : 0;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian9.Setting(containerEl).setName(t("settings.maxRetriesName")).setDesc(t("settings.maxRetriesDesc")).addText(
      (text) => text.setValue(String(this.plugin.settings.maxRetries)).onChange(async (value) => {
        const parsed = Number.parseInt(value, 10);
        this.plugin.settings.maxRetries = Number.isFinite(parsed) ? Math.min(20, Math.max(0, parsed)) : 8;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian9.Setting(containerEl).setName(t("settings.fullHistoryName")).setDesc(t("settings.fullHistoryDesc")).addButton(
      (button) => button.setButtonText(t("settings.fullHistoryButton")).onClick(async () => {
...
var AUTO_SYNC_MIN_PERIOD_MS = 12 * HOUR_MS;
var AUTO_SYNC_CHECK_INTERVAL_MS = HOUR_MS;
function isAutoSyncDue(lastSyncIso, nowMs = Date.now(), minPeriodMs = AUTO_SYNC_MIN_PERIOD_MS) {
  if (!lastSyncIso) {
    return true;
  }
...
var ReadwiseTrackerPlugin = class extends import_obsidian10.Plugin {
  ...
  async syncReadwiseData(options) {
    if (!this.settings.readwiseToken) {
      if (!(options == null ? void 0 : options.silent)) {
        this.notice(t("notice.setToken"));
      }
    ...
    if (!(options == null ? void 0 : options.silent)) {
      this.notice(t((options == null ? void 0 : options.fullHistory) ? "notice.syncingFullHistory" : "notice.syncing"));
    }
    this.syncInFlight = (async () => {
      try {
        if (options == null ? void 0 : options.fullHistory) {
...
var ReadwiseTrackerPlugin_default = ReadwiseTrackerPlugin;
